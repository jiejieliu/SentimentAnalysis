...
#
#
# neg = yuchuli('../sentiment analysis/dictionary/data_neg.txt', -1)
# pos = yuchuli('../sentiment analysis/dictionary/data_pos.txt', 1)
#
# mydata = pd.concat([neg, pos], ignore_index=True)[['comment', 'mark']]  # 结果文件
# # 预处理基本结束

mydata = pd.read_csv('../sentiment analysis/dictionary/BosonNLP_sentiment_score.txt', header=None, encoding='utf-8',
                     sep=' ', names=['comment', 'mark', 'result'])
...
nodict = []  # 否定词词典
plusdict = []  # 程度副词词典
sl = pd.read_csv('../sentiment analysis/dictionary/neg.txt', header=None, encoding='utf-8')
for i in range(len(sl[0])):
    negdict.append(sl[0][i])
sl = pd.read_csv('../sentiment analysis/dictionary/pos.txt', header=None, encoding='utf-8')
for i in range(len(sl[0])):
    posdict.append(sl[0][i])
sl = pd.read_csv('../sentiment analysis/dictionary/no.txt', header=None, encoding='utf-8')
for i in range(len(sl[0])):
    nodict.append(sl[0][i])
sl = pd.read_csv('../sentiment analysis/dictionary/plus.txt', header=None, encoding='utf-8')
for i in range(len(sl[0])):
...
for i in range(len(mydata)):
    print(i)
    tol = tol + 1
    if predict(mydata.loc[i, 'comment'], negdict, posdict, nodict, plusdict) * float(mydata.loc[i, 'mark']) > 0:
        yes = yes + 1
